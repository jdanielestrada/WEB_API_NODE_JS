...
var pos_dao = {
    ...
    getPlantilla: function (parametro, callback) {
        ...
        var connection = new sql.Connection(utils.clone(config.configBD2), function (err) {
            ...
            request.execute('POSMADECENTRO.SSP_CORPORATIVA_PLANTILLAS_EMAIL', function (err, recordsets, returnValue) {
                if (err) {
                    callback(err);
                } else {
                    self.RESULTS_POS_DAO = recordsets;
    ...
    getPlantillaByConectorSiesa: function (conectorsiesa) {
        return new Promise((resolve, reject) => {
            ...
            var connection = new sql.Connection(utils.clone(config.configBD2), function (err) {
                ...
                request.execute('POSMADECENTRO.SSP_GET_M_CONECTORES_SIESA_BY_CONECTOR',
                    function (err, recordsets, returnValue) {
                        if (err) {
                            reject(err);
                        }
